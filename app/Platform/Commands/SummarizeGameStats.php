...
class SummarizeGameStats extends Command
{
    ...
    {
        if ($game[$this->consoleIndex] === 100 || $game[$this->consoleIndex] === 101) {
            return true;
        }
    ...
    private function annualSummary(int $count): void
    {
        ...
        $query = "SELECT achievement_id, count(*) AS num
                  ...
                  WHERE unlocked_hardcore_at >= '$year-01-01'
        ...
        $query = "SELECT game_id, SUM(duration) AS mins
                  ...
                  AND duration >= 5
                  GROUP BY game_id HAVING num > 10
                  ORDER BY mins DESC
                  LIMIT 100";
        ...
        $query = "SELECT game_id, COUNT(distinct user_id) AS num
                  ...
                  WHERE created_at >= '$year-01-01'
                  AND duration >= 5
                  AND game_id IN (" . explode(',', $game_ids) . ")
                  GROUP BY game_id";
        $players = [];
        foreach (DB::select($query) as $row) {
        ...
        foreach ($stats as $row) {
            $gameData = $this->findGameData($row->game_id);
            echo str_pad("{$row->mins}", 6, ' ', STR_PAD_LEFT);
            echo " ";
